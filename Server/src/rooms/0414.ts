      ...
      try {     
          ...
          if (consented) {   
           ...
           // throw new ServerError(400,"手动断线了");  
            ...
            const newClient = await this.allowReconnection(client, 600);
            this.resumeAllTimer();
           // throw new ServerError(400,"我踢了他");  
           this.broadcast("messages", {action:'reconnected',sessionId:newClient.id});
             //client.send("reconnected", {data : "reconnected"});
  ...
  */  
    ...
    waitNightAction(){     
      ...
      this.dayInterval = this.clock.setInterval(() => {
          ...
          logger.warn("夜晚倒计时:"+delay)
      }, 1000);   
      
     // this.setProgress(62);
        
      // this.nightDelay(this.state.nightDelay,"autoNightAction")      
    }
    waitDayAction(){     
...
//      this.waitDuskAction()
    ...
    //62
    waitDuskAction(){
        var  delay =  this.state.duskDelay;
              // 10 秒過後清理計時器;
        this.state.serverTimeout = delay;
        this.dayTimeout =this.clock.setTimeout(() => {
          this.dayTimeout.clear();
      ...
      if (!player) {
        ...
        switch (message.action) {
          ...
          case 'startNominate':
              ...
              var seatId = message.targets[0];
              console.log("startNominate, "+seatId)
            // if (nominateManager.voting) return;
