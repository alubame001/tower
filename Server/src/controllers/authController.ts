...
export async function signUp(req: any, res: any) {
    try {
        ...
        if (!req.body.username || !req.body.password) {
            ...
            logger.error(`*** Sign Up Error - New user must have a username, and password!..........`);
...
export async function logIn(req: any, res: any) {
    try {
        ...
        if (!req.body.username || !req.body.password) {
            ...
            throw new ServerError(400, "Missing username or password");
            return;
        }

        // Check if an account with the email exists
        let user: User = await userRepo.findOne({ username: req.body.username });
...
*/
        ...
        logger.silly("Date.now() - user.pendingSessionTimestamp ", Date.now() - user.pendingSessionTimestamp)
...
export async function auth(options:any) {
    try {
        ...
        // Check if passwords match
        let validPassword: boolean = await compareEncrypted(options.password, user.password);
